...
- **ダブルアップ**: 勝利後、配当を2倍に賭けられます（最大5連続）。←/→キーまたは1-4キーでカードを選び、Enterで決定。同点の場合は「引き分け」で配当維持のまま再挑戦可能
...
|---|---|
| `←` / `→` | カード選択（ダブルアップでも使用） |
| `1`〜`5` | カード交換画面で該当カードを直接トグル |
| `1`〜`4` | ダブルアップ画面で該当カードを直接選択 |
| `Space` | 選択中のカードを交換する/しないを切替 |
| `a` | カード交換画面ですべて交換 / ベット入力時に実績一覧を表示 |
| `k` | カード交換をすべて解除 |
| `p` | ベット入力時に配当表を表示 |
