...
test("playDoubleUp returns true when player card is higher", () => {
  const dealerCard = { rank: "5", suit: "S" };
  const playerCard = { rank: "A", suit: "H" };
  assert.equal(playDoubleUp(dealerCard, playerCard), true);
});

test("playDoubleUp returns false when player card is lower", () => {
  const dealerCard = { rank: "K", suit: "S" };
  const playerCard = { rank: "2", suit: "H" };
  assert.equal(playDoubleUp(dealerCard, playerCard), false);
});

test("playDoubleUp returns false when player card is same rank", () => {
...
test("playDoubleUp works with edge ranks", () => {
  assert.equal(playDoubleUp({ rank: "2", suit: "S" }, { rank: "3", suit: "H" }), true);
  assert.equal(playDoubleUp({ rank: "A", suit: "S" }, { rank: "K", suit: "H" }), false);
  assert.equal(playDoubleUp({ rank: "Q", suit: "S" }, { rank: "A", suit: "H" }), true);
});
