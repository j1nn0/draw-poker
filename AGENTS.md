...
- `getPayTable()` returns the Jacks or Better 1-coin pay table. `calculatePayout(handName)` returns the payout for a given hand name.
...
- Tests are intentionally minimal: they cover scoring, hold-input parsing, hand ranking, draw behavior, and persistence, but do not cover every edge case in the CLI loop or double-up flow.
